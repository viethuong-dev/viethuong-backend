...
export class AuthService {
  constructor(private jwtService: JwtService, @InjectModel(UserRefreshToken.name) private userRefreshTokenModel: Model<UserRefreshToken>) {}
  ...
  async generateToken(user: User): Promise<Token> {
    const payload: Payload = { username: user.username, userid: user._id.toHexString(), roles: user.roles };
    return {
      access_token: this.signJWT(payload, this.accessTokenExpriesIn, this.accessTokenSecret),
      refresh_token: this.signJWT(payload, this.refreshTokenExpriesIn, this.refreshTokenSecret),
    };
  }
  ...
  async refreshToken(payload: Payload, refreshToken: string): Promise<Token> {
    ...
    return {
      access_token: this.signJWT(newPayload, this.accessTokenExpriesIn, this.accessTokenSecret),
      refresh_token: refreshToken,
    };
  }

  async createUserRefreshToken(refreshToken: string, userId: Types.ObjectId): Promise<UserRefreshToken> {
    const userRefreshToken: UserRefreshToken = {
      user_id: userId,
      refresh_token: refreshToken,
      expired_at: new Date(this.refreshTokenExpriesIn * 1000 + new Date().getTime()),
    };
